...
IMAGENS_PASSARO_PLAYER = [
    pygame.transform.scale2x(
        pygame.image.load(os.path.join(caminho_imgs, "bird-player1.png"))
    ),
    pygame.transform.scale2x(
        pygame.image.load(os.path.join(caminho_imgs, "bird-player2.png"))
    ),
    pygame.transform.scale2x(
        pygame.image.load(os.path.join(caminho_imgs, "bird-player3.png"))
    ),
]
...
def desenhar_tela(tela, passaros, player, canos, chao, pontos, tick, hs, populacao):
    tela.blit(
    ...
    if len(player) > 0:
        player[0].desenhar(tela)

    for cano in canos:  # Desenha todos os canos passados por parâmetro
        cano.desenhar(tela)

    if ia_jogando:
        # Desenha o texto da Geração
        gen = FONTE_PONTOS.render(f"Geração: {geracao}", 1, (255, 255, 255))
        tela.blit(gen, (10, 10))
    ...
    vel = FONTE_PONTOS.render(f"Velocidade: {tick}", 1, (0, 0, 0))
    tela.blit(vel, ((250 - vel.get_width() / 2), (TELA_ALTURA - vel.get_height() - 75)))

    # Desenha o texto do recorde
    high_score = FONTE_PONTOS.render(f"Recorde: {hs}", 1, (255, 255, 255))
    tela.blit(
        high_score,
        (TELA_LARGURA - 10 - high_score.get_width(), (high_score.get_height() + 15)),
    )

    # Desenha o texto da população
    pop = FONTE_PONTOS.render(f"População: {populacao}", 1, (255, 255, 255))
    tela.blit(pop, (10, (10 + gen.get_height() + 10)))
...
def main(genomas, config):
    # Definindo variáveis
    global geracao
    global populacao_viva
    populacao_viva = 0
    geracao += 1

    player = [Passaro(110, 250)]
    player[0].IMGS = IMAGENS_PASSARO_PLAYER
    ...
    # Pássaro do jogador, caso a IA não esteja jogando
    else:
    ...
    rodando = True
    while rodando:
        relogio.tick(tick)  # Define o tick do game
        ...
        for evento in pygame.event.get():
            ...
            if not ia_jogando:  # Caso a IA não esteja jogando
                if evento.type == pygame.KEYDOWN:  # Caso aperte espaço, o pássaro pula
                    if evento.key == pygame.K_SPACE:
                        for passaro in passaros:
                            passaro.pular()

            if ia_jogando:  # Caso a IA esteja jogando
                if (
                    evento.type == pygame.KEYDOWN
                ):  # Caso aperte espaço, o pássaro 0 [jogador] pula
                    if evento.key == pygame.K_SPACE:
                        for player_bird in player:
                            player_bird.pular()

            # Aumenta/diminui o tick (velocidade) do game com as setas
            if (
                evento.type == pygame.KEYDOWN
        ...
        for i, player_bird in enumerate(player):
            if (
                player_bird.y + player_bird.imagem.get_height() > chao.y
                or player_bird.y < 0
            ):
                player.pop(i)
        ...
        if ia_jogando:
            ...
            indice_cano = 0
            if (
                len(passaros) > 0 or len(player) > 0
            ):  # Se houver pássaros vivos, continue
                if (
                    len(passaros) > 0
                    and len(canos) > 1
        ...
        for cano in canos:
            for i, player_bird in enumerate(player):
                if cano.colidir(player_bird):
                    player.pop(0)
                if not cano.passou and player_bird.x > cano.x:
                    cano.passou = True
                    adicionar_cano = True
            for i, passaro in enumerate(passaros):
                ...
                if cano.colidir(passaro):
                    ...
                    populacao_viva -= 1
                    if ia_jogando:
                        # Pune a IA por morrer (-3.0 treat) e exclui o genoma
                        lista_genomas[i].fitness -= 3
                        lista_genomas.pop(i)
                    else:
                        # Se a IA não estiver jogando e o jogador colidir, reinicie o game
                        rodar(caminho_config)
        ...
        if adicionar_cano:
            ...
            if ia_jogando:
                for genoma in lista_genomas:
                    # Dá um bônus (+10.0 treat) quando a IA passa pelo cano
                    genoma.fitness += 10
